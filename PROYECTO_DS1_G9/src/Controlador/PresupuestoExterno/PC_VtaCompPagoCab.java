...
import Vistas.PresupuestoExterno.PV_Venta;
...
public class PC_VtaCompPagoCab implements ActionListener,MouseListener, ItemListener{
    
    PV_Venta vVenta = new PV_Venta();
    DefaultTableModel modelVenta = new DefaultTableModel();
    TableRowSorter<DefaultTableModel> sorter;
    int fila=-1;
    int secuencia = 0;

    public PC_VtaCompPagoCab(PV_Venta vVenta){
        this.vVenta=vVenta;
        init();
    ...
    public void initListarPartidas(int codPyto){
        vVenta.codPartida.removeAllItems();
        List<DatosPartida> lista = new PProy_Partida_MezclaDAO().getv_datos_partida(varCodCiaGlobalDeLogin, codPyto, "I");
        System.out.println("Lista de Partidas: "+lista.size());
        System.out.println(varCodCiaGlobalDeLogin);
        System.out.println(codPyto);
        for(int i=0;i<lista.size();i++){
            System.out.println(lista.get(i).getDesPartida());
            vVenta.codPartida.addItem(lista.get(i));
        }
    ...
    public void agregarVentaDetalleATabla(){
        ...
        String codPart=Integer.toString(((DatosPartida)vVenta.codPartida.getSelectedItem()).getCodPartida());
    ...
    public boolean agregarCompraBD(){
        ...
        if(new PVtaComp_PagoCabDAO().add(vtaComp_PagoCab)==0){
    ...
    public void agregarDetalleCompra(){
        for(int i = 0; i < vVenta.tablaVenta.getRowCount();i++){
            ...
            if(new PVtaComp_PagoDetDAO().add(vtaComp_PagoDet)==0){
    ...
    public void itemStateChanged(ItemEvent e) {
        ...
        if(e.getSource()==vVenta.codPartida){
            if(vVenta.codPartida.getSelectedIndex()!=-1){
                DatosPartida dp = (DatosPartida) vVenta.codPartida.getSelectedItem();
                vVenta.desPartida.setText(dp.getDesPartida());
                vVenta.semilla.setText(String.valueOf(dp.getSemilla()));
                vVenta.impNetoMNPart.setText(dp.getCostoTot()+"");
                vVenta.impIgvMNPart.setText(dp.getIgv()+"");
            } else{
