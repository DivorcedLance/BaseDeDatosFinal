...
import Modelo.Auxiliares.DatosPartida;
...
public class PProy_Partida_MezclaDAO implements CRUD <Proy_Partida_Mezcla>{
    ...
    public List<Proy_Partida_Mezcla> listar() {
       List<Proy_Partida_Mezcla> lista = new ArrayList<>();
        String sql="SELECT * FROM P_PROY_PARTIDA_MEZCLA order by CODCIA";
        try{
    ...
    public int add(Proy_Partida_Mezcla p) {
        System.out.println("{call P_INSERTAR_PROY_PARTIDA_MEZCLA(?,?,?,?,?,?)}");
        try{
           con=conexion.conectar();
           myCall = con.prepareCall("{call P_INSERTAR_PROY_PARTIDA_MEZCLA(?,?,?,?,?,?)}");
    ...
    public int actualizar(Proy_Partida_Mezcla p) {
        String sql1= "update P_PROY_PARTIDA_MEZCLA set NROVERSION=?,NIVEL=?,ORDEN=?,COSTOUNIT=?,CANT=?,COSTOTOT=? where CODCIA=? AND CODPYTO=? AND INGEGR=? AND CORR=? AND CODPARTIDA=?";
        System.out.println(sql1);
        System.out.println(p.getNroVersion()); //
        System.out.println(p.getNivel()); //
        System.out.println(p.getOrden()); //
        System.out.println(p.getCostoUnit()); //
        System.out.println(p.getCant()); //
        System.out.println(p.getCostoTot()); //
        
        System.out.println(p.getCodCia()); //
        System.out.println(p.getCodPyto()); //
        System.out.println(p.getIngEgr()); //
        System.out.println(p.getCorr()); //
        System.out.println(p.getCodPartida()); //
        try{
           con=conexion.conectar();
    ...
    public void eliminar(int id) {
       String sql1="DELETE from P_PROY_PARTIDA_MEZCLA where CORR="+id;
        try{
            con=conexion.conectar();
    ...
    public List<Proy_Partida_Mezcla> listarPorCodCia(int id, String tip){
        List<Proy_Partida_Mezcla> lista = new ArrayList<>();
        String sql="SELECT "
                + "codpyto,nroversion,CORR,codpartida,padcodpartida,nivel,orden,tunimed,eunimed,costounit,cant,costotot "
                + "FROM P_PROY_PARTIDA_MEZCLA WHERE codcia="+id+" AND ingegr='"+tip+"' order by CORR";
    ...
    public List<Proy_Partida_Mezcla> listarCodPartida_Mezcla(int id,String tip,int pyto){
        List<Proy_Partida_Mezcla> lista = new ArrayList<>();
        String sql="SELECT "
                + "codpartida"
                + " FROM P_PROY_PARTIDA_MEZCLA WHERE codcia="+id+" AND ingegr='"+tip+"' AND CODPYTO="+pyto+" order by codpartida";
        System.out.println(sql);
        try{
            con=conexion.conectar();
            ps=con.prepareStatement(sql);
    ...
    public List<Proy_Partida_Mezcla> listarNiveles(int codC, int codProy, String tipo){
        List<Proy_Partida_Mezcla> lista = new ArrayList<>();
        String sql="SELECT CODPARTIDA, NIVEL FROM P_PROY_PARTIDA_MEZCLA WHERE codcia="+codC+" AND codpyto="+codProy+" AND ingegr='"+tipo+"' order by CORR";
        System.out.println(sql);
        try{
            con=conexion.conectar();
            ps=con.prepareStatement(sql);
    ...
    public Proy_Partida_Mezcla listarId(int cia,String tipo, int id,int corr,int ver,int pyto){
        ...
        String sql="SELECT CODPYTO,NROVERSION,CODPARTIDA,PADCODPARTIDA,NIVEL,ORDEN,COSTOUNIT,"
                + "CANT FROM P_PROY_PARTIDA_MEZCLA WHERE CODCIA="+cia+" AND INGEGR='"+tipo+"' "
    ...
    public Proy_Partida_Mezcla listarId2(int cia,String tipo, int id,int pyto){
        ...
        String sql="SELECT NROVERSION,CORR,ORDEN"
                + " FROM P_PROY_PARTIDA_MEZCLA WHERE CODCIA="+cia+" AND INGEGR='"+tipo+"' "
    ...
    public void eliminarDatos(int cia,int cod,String tipo,int corr,int pyto,int ver) {
       String sql1="DELETE from P_PROY_PARTIDA_MEZCLA where CODCIA="+cia+" AND"
               + " INGEGR='"+tipo+"' AND CODPARTIDA="+cod+" AND CORR="+corr+" AND CODPYTO="+pyto+" AND NROVERSION="+ver;
        try{
            con=conexion.conectar();
            ps=con.prepareStatement(sql1);
            rs=ps.executeQuery(sql1);
    ...
    public int asignarOrden(String tip,int nivel,int cia, int pyto){
        int orden=0;
        String sql="SELECT * FROM (SELECT ORDEN FROM P_PROY_PARTIDA_MEZCLA WHERE INGEGR='"+tip+"' AND NIVEL="+nivel+" AND CODCIA="+cia+" AND CODPYTO="+pyto+" ORDER BY ORDEN DESC) WHERE ROWNUM=1";
        try{
            con=conexion.conectar();
            ps=con.prepareStatement(sql);
            rs=ps.executeQuery(sql);
    ...
    public int asignarOrdenNoNULL(String tip,int cod,int cia,int pyto){
        int orden=0;
        String sql="SELECT * FROM (SELECT ORDEN FROM P_PROY_PARTIDA_MEZCLA WHERE INGEGR='"+tip+"' AND PADCODPARTIDA="+cod+" AND CODCIA="+cia+" AND CODPYTO="+pyto+" ORDER BY ORDEN DESC) WHERE ROWNUM=1";
        try{
            con=conexion.conectar();
            ps=con.prepareStatement(sql);
            rs=ps.executeQuery(sql);
    ...
    public int asignarNivelNoNULL(String tip,int cod,int cia,int pyto){
        int nivel=0;
        String sql="SELECT * FROM (SELECT NIVEL FROM P_PROY_PARTIDA_MEZCLA WHERE INGEGR='"+tip+"' AND CODPARTIDA="+cod+" AND CODCIA="+cia+" AND CODPYTO="+pyto+" ORDER BY ORDEN DESC) WHERE ROWNUM=1";
        try{
            con=conexion.conectar();
            ps=con.prepareStatement(sql);
            rs=ps.executeQuery(sql);
    ...
    public float buscarTotalCodPartCodProy(String pyto, String codpart){
        float costo=0;
        String sql="SELECT "
                + "COSTOTOT FROM P_PROY_PARTIDA_MEZCLA WHERE CODPYTO="+pyto+" AND CODPARTIDA="+codpart;
        System.out.println(sql);
        try{
            con=conexion.conectar();
            ps=con.prepareStatement(sql);
    ...
    public List<Proy_Partida_Mezcla> eliminarCodPartida(List<Proy_Partida_Mezcla> lista,int id,String tip,int pyto){
        
        String sql="SELECT DISTINCT PADCODPARTIDA FROM P_PROY_PARTIDA_MEZCLA WHERE CODCIA="+id+" AND "
    ...
    public List getv_proy_partida_mezcla(int codCia, String ingEgr) {
        List lista = new ArrayList<>();
        List<Proy_Partida_Mezcla> listaPpm = new ArrayList<>();
        List<ProyectoAux> listaPyto = new ArrayList<>();
        List<PartidaAux> listaPartida = new ArrayList<>();
        List<PartidaAux> listaPartidaPadre = new ArrayList<>();

        String sql = "{call p_getv_proy_partida_mezcla(?, ?, ?)}";
    ...
    public List<DatosPartida> getv_datos_partida(int codCia, int codPyto, String ingEgr) {
        List<DatosPartida> lista = new ArrayList<>();

        String sql = "{call p_getv_datos_partida(?, ?, ?, ?)}";
        ...
        try (Connection con = conexion.conectar();
            ...
            try (ResultSet rs = (ResultSet) cs.getObject(4)) {
                while (rs.next()) {
                    ...
                    dp.setCodCia(codCia);
                    dp.setCodProyecto(codPyto);
                    dp.setIngegr(ingEgr);
                    dp.setCodPartida(rs.getInt("CODPARTIDA"));
                    dp.setDesPartida(rs.getString("DESPARTIDA"));
                    dp.setCostoTot(rs.getFloat("COSTOTOT"));
                    dp.setIgv(rs.getFloat("IGV"));
                    dp.setSemilla(rs.getString("SEMILLA"));
                    lista.add(dp);
                    System.out.println("T: "+dp.toString());
                }
            }
        } catch (SQLException e) {
            System.out.println(e.toString());
