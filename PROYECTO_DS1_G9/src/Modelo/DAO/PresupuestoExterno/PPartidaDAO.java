...
public class PPartidaDAO implements CRUD<Partida>{
    ...
    public List<Partida> listar() {
        ...
        String sql="SELECT * FROM P_PARTIDA order by CODCIA";
        try{
    ...
    public int add(Partida p) {
        System.out.println("{call P_INSERTAR_PARTIDA(?,?,?,?,?,?)}");
        try{
           con=conexion.conectar();
           myCall = con.prepareCall("{call P_INSERTAR_PARTIDA(?,?,?,?,?,?)}");
    ...
    public int actualizar(Partida p) {
            String sql1= "update P_PARTIDA set DesPartida=?,TUniMed=?,EUniMed=?,Vigente=? where CodCIA=? AND CodPartida=? AND IngEgr=?";
    ...
    public void eliminarDatos(int cia, int cod, String tip) {
       String sql1="DELETE from P_PARTIDA where CodCIA="+cia+" AND CodPartida="+cod+" AND IngEgr='"+tip+"'";
        try{
            con=conexion.conectar();
    ...
    public Partida listarId(int cia,int id, String tip){
        Partida pm = new Partida();
        String sql="SELECT DesPartida,tUniMed,eUniMed,vigente,CODPARTIDAS FROM P_PARTIDA WHERE CODCIA="+cia+" AND CODPARTIDA="+id+" AND INGEGR='"+tip+"'";
        try{
            con = conexion.conectar();
    ...
    public Partida listarIdv2(int cia,int id){
        Partida pm = new Partida();
        String sql="SELECT DesPartida,tUniMed,eUniMed,vigente,CODPARTIDAS FROM P_PARTIDA WHERE CODCIA="+cia+" AND CODPARTIDA="+id;
        try{
            con = conexion.conectar();
            ps=con.createStatement();
            rs = ps.executeQuery(sql);
    ...
    public String listarDescPartida(int cia,int id, String tip){
        ...
        String desc="";
        String sql="SELECT DesPartida FROM P_PARTIDA WHERE CODCIA="+cia+" AND CODPARTIDA="+id+" AND INGEGR='"+tip+"'";
        try{
            con = conexion.conectar();
            ps=con.createStatement();
            rs = ps.executeQuery(sql);
    ...
    public List<Partida> listarPorCodCia(int id,String tip){
        ...
        String sql="SELECT "
                ...
                + "FROM P_PARTIDA p WHERE p.CodCIA="+id+" AND p.IngEgr='"+tip+"' order by p.codPartida";
     ...
     public List listarPartidas(String id) {
        List<Partida> lista = new ArrayList<>();
        String sql="SELECT * FROM P_PARTIDA WHERE INGEGR="+id;
        try{
            con=conexion.conectar();
            ps=con.prepareStatement(sql);
            rs=ps.executeQuery(sql);
