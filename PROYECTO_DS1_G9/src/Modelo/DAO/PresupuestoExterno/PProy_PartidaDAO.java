...
public class PProy_PartidaDAO implements CRUD<Proy_Partida> {
    ...
    public List<Proy_Partida> listar() {
        ...
        String sql = "SELECT * FROM P_PROY_PARTIDA order by CODCIA";
        try {
    ...
    public int add(Proy_Partida p) {
        System.out.println("{call P_INSERTAR_PROY_PARTIDA(?,?,?,?,?,?,?,?,?)}");
        try {
            con = conexion.conectar();
            myCall = con.prepareCall("{call P_INSERTAR_PROY_PARTIDA(?,?,?,?,?,?,?,?,?)}");
    ...
    public int actualizar(Proy_Partida p) {
        String sql1 = "update P_PROY_PARTIDA set NROVERSION=? where CODCIA=? AND CODPYTO=? AND INGEGR=? and CODPARTIDA=?";
    ...
    public void eliminarDatos(int cia, int cod, String tip, int pyto, int ver) {
        String sql1 = "DELETE from P_PROY_PARTIDA where CODCIA=" + cia + " AND CODPARTIDA=" + cod + " AND INGEGR='" + tip
        ...
        try {
            ...
            ps.close();
            con.close();
            JOptionPane.showMessageDialog(null, "P_Proy_Partida eliminado con exito.");
        } catch (Exception e) {
            JOptionPane.showMessageDialog(null, "Excepcion.\n" + e.toString());
            System.out.println(e.toString());
        }
    ...
    public Proy_Partida listarId(int cia, int pyto, int ver, int cod, String tip) {
        Proy_Partida pm = new Proy_Partida();
        String sql = "SELECT CODPYTO,NROVERSION,CODPARTIDA,CODESTADO,VIGENTE FROM P_PROY_PARTIDA WHERE CODCIA=" + cia
                + " AND"
    ...
    public List<Proy_Partida> listarPorCodCia(int id, String tip) {
        ...
        String sql = "SELECT "
                ...
                + "FROM P_PROY_PARTIDA WHERE CODCIA=" + id + " AND INGEGR='" + tip + "' order by CODPYTO";
    ...
    public List<Proy_Partida> listarPorCodCiav2(int id, String tip, int pyto) {
        ...
        String sql = "SELECT "
                + "CodPyto,NroVersion,CodPartida,CodEstado,Vigente "
                + "FROM P_PROY_PARTIDA WHERE CODCIA=" + id + " AND INGEGR='" + tip + "' AND CODPYTO=" + pyto
                + " order by CODPYTO";
        System.out.println(sql);
        try {
            con = conexion.conectar();
    ...
    public List<Proy_Partida> listarPorCodPyto(int id) {
        ...
        String sql = "SELECT "
                + "CodPartida "
                + "FROM P_PROY_PARTIDA WHERE CODPYTO=" + id + " order by CODPARTIDA";
        System.out.println(sql);
        try {
            con = conexion.conectar();
            ps = con.prepareStatement(sql);
    ...
    public Proy_Partida listarId(String id) {
        ...
        String sql = "SELECT DESPARTIDA,CODPARTIDA FROM P_PROY_PARTIDA WHERE CODPARTIDA=" + id + " order by CODPARTIDA";
        try {
            con = conexion.conectar();
            ps = con.createStatement();
            rs = ps.executeQuery(sql);
    ...
    public List getv_proy_partida(int codCia, String ingEgr) {
        ...
        String sql = "{call p_getv_proy_partida(?, ?, ?)}";
        try (Connection con = conexion.conectar();
             CallableStatement cs = con.prepareCall(sql)) {
