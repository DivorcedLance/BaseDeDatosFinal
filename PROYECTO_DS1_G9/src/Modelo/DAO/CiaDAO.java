...
public class CiaDAO implements CRUD<Cia>{
        ...
        try{
            ...
            while(rs.next()){
                Cia c = new Cia.Builder()
                        ...
                        .build();
                System.out.println(c.getCodCia()+" "+c.getDesCia()+" "+c.getDesCorta()+" "+c.getVigente());
                lista.add(c);
            }
            rs.close();
            ps.close();
    ...
    public List<Cia> GetAllCia(){
        List<Cia> lista = new ArrayList<>();
        String sql = "{call get_all_cia(?)}";
        ...
        try (Connection con = conexion.conectar();
            ...
            try (ResultSet rs = (ResultSet) cs.getObject(1)) {
                while (rs.next()) {
                    Cia c = new Cia.Builder()
                            .codCia(rs.getInt("CODCIA"))
                            .desCia(rs.getString("DESCIA"))
                            .desCorta(rs.getString("DESCORTA"))
                            .vigente(rs.getCharacterStream("VIGENTE").toString().charAt(0))
                            .build();
        ...
        System.out.println("Terminando la lista Mezcla");
        return lista;
    }
