...
create or replace NONEDITIONABLE PROCEDURE INSERTAR_VTACOMP_PAGODET(
    COD_CIA IN VTACOMP_PAGODET.codcia%TYPE,
...
FECHA_DE_CP DATE;
BEGIN
    INSERT INTO VTACOMP_PAGODET VALUES(COD_CIA,NRO_CP,SEC,INGEGR,CODPARTIDA,IMPNETOMN,IMPIGVMN,IMPTOTALMN,SEMILL);
    SELECT FECCP INTO FECHA_DE_CP FROM VTACOMP_PAGOCAB WHERE NROCP=NRO_CP
    AND CODCIA=COD_CIA;
    UPDATE_FLUJOCAJA_DET_UNALINEA_Y_GLOBAL(COD_CIA,COD_PYTO_AUXILIAR,INGEGR,CODPARTIDA,IMPTOTALMN,FECHA_DE_CP,'R');
END;
/

create or replace procedure getv_partidas_mezcla(
    pm_cod_cia in PARTIDA_MEZCLA.CODCIA%type,
    pm_ie in PARTIDA_MEZCLA.INGEGR%type,
    v_cursor out sys_refcursor
) as
begin
    open v_cursor for
        select PARTIDA_MEZCLA.corr,
               PARTIDA_MEZCLA.CODPARTIDA,
               PARTIDA.DESPARTIDA                             as DESPARTIDA,
               PARTIDA_MEZCLA.PADCODPARTIDA,
               (SELECT PARTIDA.DESPARTIDA
                FROM PARTIDA
                WHERE PARTIDA.CODPARTIDA = PARTIDA_MEZCLA.PADCODPARTIDA
                  AND PARTIDA.CODCIA = PARTIDA_MEZCLA.CODCIA
                  and PARTIDA.INGEGR = PARTIDA_MEZCLA.INGEGR) AS DESCPADCODPARTIDA,
               PARTIDA_MEZCLA.NIVEL,
               PARTIDA_MEZCLA.ORDEN,
               PARTIDA_MEZCLA.TUNIMED,
               TABS.DENTAB,
               PARTIDA_MEZCLA.EUNIMED,
               ELEMENTOS.DENELE,
               PARTIDA_MEZCLA.COSTOUNIT
        from PARTIDA_MEZCLA
                 join PARTIDA on PARTIDA_MEZCLA.CODCIA = PARTIDA.CODCIA
            and PARTIDA_MEZCLA.CODPARTIDA = PARTIDA.CODPARTIDA
            and PARTIDA_MEZCLA.INGEGR = PARTIDA.INGEGR
                 join TABS on TABS.CODTAB = PARTIDA_MEZCLA.TUNIMED
                 join ELEMENTOS on ELEMENTOS.CODELEM = PARTIDA_MEZCLA.EUNIMED and
                                   ELEMENTOS.CODTAB = PARTIDA_MEZCLA.TUNIMED
        where PARTIDA_MEZCLA.INGEGR = pm_ie
          and PARTIDA_MEZCLA.CODCIA = pm_cod_cia
        order by PARTIDA_MEZCLA.PADCODPARTIDA, PARTIDA_MEZCLA.NIVEL,
                 PARTIDA_MEZCLA.ORDEN;
end;
/
...
create or replace procedure getv_proy_partida(
    pp_cod_cia in PROY_PARTIDA.CODCIA%type,
    pp_ie in PROY_PARTIDA.INGEGR%type,
    v_cursor out sys_refcursor
) as
begin
    open v_cursor for
        select PROY_PARTIDA.CODPYTO,
               PROYECTO.NOMBPYTO,
               PROY_PARTIDA.CODPARTIDA,
               PARTIDA.DESPARTIDA,
               PROY_PARTIDA.CODPARTIDAS,
               PROY_PARTIDA.VIGENTE
        from PROY_PARTIDA
                 join PROYECTO on PROY_PARTIDA.CODCIA = PROYECTO.CODCIA
            and PROY_PARTIDA.CODPYTO = PROYECTO.CODPYTO
                 join PARTIDA on PROY_PARTIDA.CODCIA = PARTIDA.CODCIA
            and PROY_PARTIDA.CODPARTIDA = PARTIDA.CODPARTIDA
            and PROY_PARTIDA.INGEGR = PARTIDA.INGEGR
        where PROY_PARTIDA.INGEGR = pp_ie
          and PROY_PARTIDA.CODCIA = pp_cod_cia;
end;
/

create or replace procedure getv_proy_partida_mezcla(
    ppm_cod_cia in PROY_PARTIDA_MEZCLA.CODCIA%type,
    ppm_ie in PROY_PARTIDA_MEZCLA.INGEGR%type,
    v_cursor out sys_refcursor
) as
begin
    open v_cursor for
        select PROY_PARTIDA_MEZCLA.corr,
               PROY_PARTIDA_MEZCLA.CODPYTO,
               PROYECTO.NOMBPYTO,
               PROY_PARTIDA_MEZCLA.CODPARTIDA,
               PARTIDA.DESPARTIDA                                  as DESPARTIDA,
               PROY_PARTIDA_MEZCLA.PADCODPARTIDA,
               (select PARTIDA.DESPARTIDA
                from PARTIDA
                where PARTIDA.CODPARTIDA = PROY_PARTIDA_MEZCLA.PADCODPARTIDA
                  and PARTIDA.CODCIA = PROY_PARTIDA_MEZCLA.CODCIA
                  and PARTIDA.INGEGR = PROY_PARTIDA_MEZCLA.INGEGR) AS DESCPADCODPARTIDA,
               PROY_PARTIDA_MEZCLA.NIVEL,
               PROY_PARTIDA_MEZCLA.ORDEN,
               PROY_PARTIDA_MEZCLA.TUNIMED,
               TABS.DENTAB,
               PROY_PARTIDA_MEZCLA.EUNIMED,
               ELEMENTOS.DENELE,
               PROY_PARTIDA_MEZCLA.COSTOUNIT,
               PROY_PARTIDA_MEZCLA.CANT,
               PROY_PARTIDA_MEZCLA.COSTOTOT
        from PROY_PARTIDA_MEZCLA
                 join PROYECTO on PROY_PARTIDA_MEZCLA.CODCIA = PROYECTO.CODCIA
            and PROY_PARTIDA_MEZCLA.CODPYTO = PROYECTO.CODPYTO
                 join PARTIDA on PROY_PARTIDA_MEZCLA.CODCIA = PARTIDA.CODCIA
            and PROY_PARTIDA_MEZCLA.CODPARTIDA = PARTIDA.CODPARTIDA
            and PROY_PARTIDA_MEZCLA.INGEGR = PARTIDA.INGEGR
                 join TABS on TABS.CODTAB = PROY_PARTIDA_MEZCLA.TUNIMED
                 join ELEMENTOS on ELEMENTOS.CODELEM = PROY_PARTIDA_MEZCLA.EUNIMED and
                                   ELEMENTOS.CODTAB = PROY_PARTIDA_MEZCLA.TUNIMED
        where PROY_PARTIDA_MEZCLA.INGEGR = ppm_ie
          and PROY_PARTIDA_MEZCLA.CODCIA = ppm_cod_cia
        order by PROY_PARTIDA_MEZCLA.CODPYTO, PROY_PARTIDA_MEZCLA.PADCODPARTIDA, PROY_PARTIDA_MEZCLA.NIVEL,
                 PROY_PARTIDA_MEZCLA.ORDEN;

end;
/

create or replace procedure getv_proyecto(
    py_cod_cia in PROYECTO.CODCIA%type,
    v_cursor out sys_refcursor
) as
begin
    open v_cursor for
        select PROYECTO.CODPYTO,
               PROYECTO.NOMBPYTO,
               PROYECTO.EMPLJEFEPROY,
               (select PERSONA.DESPERSONA
                from PERSONA
                where PERSONA.CODPERSONA = PROYECTO.EMPLJEFEPROY
                  and PERSONA.CODCIA = PROYECTO.CODCIA) as DESEMPLJEFEPROY,
               PROYECTO.CODCIA,
               PROYECTO.CIACONTRATA,
               (select PERSONA.DESPERSONA
                from PERSONA
                where PERSONA.CODPERSONA = PROYECTO.CIACONTRATA
                  and PERSONA.CODCIA = PROYECTO.CODCIA) as DESCIACONTRATA,
               PROYECTO.CODCLIENTE,
               (select PERSONA.DESPERSONA
                from PERSONA
                where PERSONA.CODPERSONA = PROYECTO.CODCLIENTE
                  and PERSONA.CODCIA = PROYECTO.CODCIA) as DESCLIENTE,
               PROYECTO.FECREG,
               PROYECTO.ESTPYTO,
               PROYECTO.FECESTADO,
               PROYECTO.COSTOTOTAL,
               ...
               PROYECTO.VIGENTE
        from PROYECTO
        where PROYECTO.CODCIA = py_cod_cia;
end;
/

create or replace procedure getv_proyecto_form(
    py_cod_cia in PROYECTO.CODCIA%type,
    py_cod_proy in PROYECTO.CODPYTO%type,
    v_cursor out sys_refcursor
) as
begin
    open v_cursor for
        ...
        from PROYECTO
        where PROYECTO.CODCIA = py_cod_cia
          and PROYECTO.CODPYTO = py_cod_proy;
end;
/

create or replace procedure getv_datos_partida(
    pv_cod_cia in PARTIDA.CODCIA%type,
    pv_cod_proy in PROY_PARTIDA.CODPYTO%type,
    pv_ie in PARTIDA.INGEGR%type,
    v_cursor out sys_refcursor
) as
begin
    open v_cursor for
        select distinct PROY_PARTIDA_MEZCLA.CODPARTIDA,
                        PARTIDA.DESPARTIDA,
                        PROY_PARTIDA_MEZCLA.COSTOTOT,
                        (0.18 * PROY_PARTIDA_MEZCLA.COSTOTOT) as IGV,
                        PARTIDA.SEMILLA
        from PROY_PARTIDA_MEZCLA
                 join PARTIDA on PROY_PARTIDA_MEZCLA.CODCIA = PARTIDA.CODCIA
            and PROY_PARTIDA_MEZCLA.CODPARTIDA = PARTIDA.CODPARTIDA
            and PROY_PARTIDA_MEZCLA.INGEGR = PARTIDA.INGEGR
        where PROY_PARTIDA_MEZCLA.CODCIA = pv_cod_cia
          and PROY_PARTIDA_MEZCLA.CODPYTO = pv_cod_proy
          and PROY_PARTIDA_MEZCLA.INGEGR = pv_ie
          and PROY_PARTIDA_MEZCLA.CODPARTIDA not in (select PADCODPARTIDA
                                                     from PROY_PARTIDA_MEZCLA
...
create or replace procedure getv_datos_partidas_padre(
    pv_cod_cia in PARTIDA.CODCIA%type,
    pv_ingegr in PARTIDA.INGEGR%type,
    v_cursor out sys_refcursor
) as
begin
    open v_cursor for
        select distinct PADCODPARTIDA,
                        (select DESPARTIDA
                         from PARTIDA
                         where CODPARTIDA = PARTIDA_MEZCLA.PADCODPARTIDA
                           AND CODCIA = PARTIDA_MEZCLA.CODCIA
                           AND INGEGR = PARTIDA_MEZCLA.INGEGR) as DESPARTIDA,
                        (select INGEGR
                         from PARTIDA
                         where CODPARTIDA = PARTIDA_MEZCLA.PADCODPARTIDA
                           AND CODCIA = PARTIDA_MEZCLA.CODCIA
                           AND INGEGR = PARTIDA_MEZCLA.INGEGR) as INGEGR
        from PARTIDA_MEZCLA
...
create or replace procedure getv_datos_proyecto_partidas_padre(
    pv_cod_cia in PROY_PARTIDA.CODCIA%type,
    pv_ingegr in PROY_PARTIDA.INGEGR%type,
    pv_cod_proy in PROY_PARTIDA.CODPYTO%type,
    v_cursor out sys_refcursor
) as
begin
    open v_cursor for
        select distinct PADCODPARTIDA,
                        (select DESPARTIDA
                         from PARTIDA
                         where CODPARTIDA = PROY_PARTIDA_MEZCLA.PADCODPARTIDA) as DESPARTIDA,
                        (select INGEGR
                         from PARTIDA
                         where CODPARTIDA = PROY_PARTIDA_MEZCLA.PADCODPARTIDA) as INGEGR
        from PROY_PARTIDA_MEZCLA
        where INGEGR = pv_ingegr
          and CODCIA = pv_cod_cia
          and CODPYTO = pv_cod_proy;
...
create or replace procedure get_all_cia(
    v_cursor out sys_refcursor
) as
begin
    open v_cursor for
        select * from CIA order by CODCIA DESC;
end;
/
...
INSERT INTO PROY_PARTIDA VALUES(1,1,1,'I',1,'1','1',1,'1','-1','1','1');
INSERT INTO PROY_PARTIDA VALUES(1,1,1,'I',2,'2','1',1,'1','-1','1','1');

INSERT INTO PROY_PARTIDA VALUES(1,1,1,'E',1,'3','1',1,'1','-1','1','1');
INSERT INTO PROY_PARTIDA VALUES(1,1,1,'E',2,'4','1',1,'1','-1','1','1');
INSERT INTO PROY_PARTIDA VALUES(1,1,1,'E',3,'5','1',1,'1','-1','1','1');
INSERT INTO PROY_PARTIDA VALUES(1,1,1,'E',4,'6','1',1,'1','-1','1','1');
INSERT INTO PROY_PARTIDA VALUES(1,1,1,'E',5,'7','1',1,'1','-1','1','1');
INSERT INTO PROY_PARTIDA VALUES(1,1,1,'E',6,'8','1',1,'1','-1','1','1');
INSERT INTO PROY_PARTIDA VALUES(1,1,1,'E',7,'9','1',1,'1','-1','1','1');
INSERT INTO PROY_PARTIDA VALUES(1,1,1,'E',8,'10','1',1,'1','-1','1','1');
INSERT INTO PROY_PARTIDA VALUES(1,1,1,'E',9,'11','1',1,'1','-1','1','1');
INSERT INTO PROY_PARTIDA VALUES(1,1,1,'E',10,'12','1',1,'1','-1','1','1');

INSERT INTO PROY_PARTIDA_MEZCLA VALUES(1,1,'I',1,1,SEC_PROY_PARTIDA_MEZCLA_I.nextval,0,'2','2',1,1,1000,1,1000);
INSERT INTO PROY_PARTIDA_MEZCLA VALUES(1,1,'I',1,2,SEC_PROY_PARTIDA_MEZCLA_I.nextval,1,'2','2',2,1,2000,1,2000);
